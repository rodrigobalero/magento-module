...
class Mundipagg_Model_Observer extends Mundipagg_Model_Standard {
	/*
	...
	private function cancelOrderViaApi(Mage_Sales_Model_Order $order) {
	    $standard = new Mundipagg_Model_Standard($order->getStore());
		...
		$logHelper = new Mundipagg_Helper_Log(__METHOD__);
		$api = new Mundipagg_Model_Api($order->getStore());
    ...
    private function readModuleVersion(){
        $configXml = Mage::getBaseDir('app') . "/code/community/Mundipagg/etc/config.xml";
        if (file_exists($configXml)) {
            $xmlObj = simplexml_load_file($configXml);
            return $xmlObj->modules->Mundipagg->version[0];
        }else{
            return 0;
        }
    }
