...
class Mundipagg_Model_Standard extends Mage_Payment_Model_Method_Abstract {
            ...
            foreach ($mundipagg as $key => $value) {
                ...
                if ($pos !== false && $value != '' && $value != 'new') {
                    ...
                    if ($token->getId() && $token->getEntityId() == $customerId) {
                        ...
                        //Log error
                        Mage::log($error, null, 'Mundipagg.log');
     ...
     *
     * @return Mundipagg_Model_Session
     */
    ...
    private function offlineRetryCancelOrSuccessOrder($orderIncrementId) {
        $offlineRetryIsEnabled = Mundipagg_Model_Offlineretry::offlineRetryIsEnabled();
        $helperLog = new Mundipagg_Helper_Log(__METHOD__);
    ...
    private function integrationTimeOut(Mage_Sales_Model_Order $order, Mage_Sales_Model_Order_Payment &$payment) {
        ...
        $payment->setAdditionalInformation('IntegrationError', Mundipagg_Model_Api::INTEGRATION_TIMEOUT);

        $comment = Mundipagg_Model_Api::INTEGRATION_TIMEOUT;
