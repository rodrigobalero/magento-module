...
class Mundipagg_Model_Api extends Mundipagg_Model_Standard {
	...
	public function __construct($Store = null) {
		$this->helperUtil = new Mundipagg_Helper_Util();
		$this->modelStandard = new Mundipagg_Model_Standard($Store);
	 ...
	 */
	public function creditCardTransaction($order, $data, Mundipagg_Model_Standard $standard) {
		...
		try {
			...
			*/
			...
			/* @var $recurrencyModel Mundipagg_Model_Recurrency */
			...
			if ($helper->isAntiFraudEnabled()) {
				...
				switch ($antifraudProviderConfig) {
					case Mundipagg_Model_Source_Antifraud::ANTIFRAUD_CLEARSALE:
						$antifraudProvider = 'clearsale';
						break;

					case Mundipagg_Model_Source_Antifraud::ANTIFRAUD_FCONTROL:
						$antifraudProvider = 'fcontrol';
						break;

					case Mundipagg_Model_Source_Antifraud::ANTIFRAUD_STONE:
...
//					$trans = $creditCardTransactionResultCollection['CreditCardTransactionResult'];
	 ...
	 * Boleto transaction
	 **/
	public function boletoTransaction($order, $data, Mundipagg_Model_Standard $standard) {
		try {
			$helper = Mage::helper('mundipagg');
	 ...
	 * Debit transaction
	 **/
	public function debitTransaction($order, $data, Mundipagg_Model_Standard $standard) {
		$helperLog = new Mundipagg_Helper_Log(__METHOD__);
...
//				Mage::log(curl_error($ch), null, 'Mundipagg.log');
			}
	 ...
	 **/
	public function manageOrderRequest($data, Mundipagg_Model_Standard $standard) {
		$helperLog = new Mundipagg_Helper_Log(__METHOD__);
	...
	public function saleCapture($data, $orderReference) {
		...
		/* @var Mundipagg_Helper_Data $helper */
		$helper = Mage::helper('mundipagg');
	...
	public function processOrder($postData) {
		...
		try {
			...
			if (isset($data['OrderStatus'])) {
				...
				if ($order->getState() != Mage_Sales_Model_Order::STATE_PROCESSING && $orderStatus == Mundipagg_Model_Enum_OrderStatusEnum::CANCELED) {
			...
			/**
			 * @var $recurrence Mundiapgg_Model_Recurrency
			 */
			...
			switch ($lowerStatus) {
				...
				case $statusWithError:
					try {
						Mundipagg_Model_Standard::transactionWithError($order, false);
	...
	private function captureTransaction(Mage_Sales_Model_Order $order, $amountToCapture, $transactionKey) {
		...
		$orderPayment = new Mundipagg_Model_Order_Payment();
	...
	/**
	 * Create invoice
	 * @todo must be deprecated use Mundipagg_Model_Order_Payment createInvoice
	 * @param Mage_Sales_Model_Order $order
	 * @param array $data
	...
	public function getTransactionHistory($orderKey) {
		...
		// @var $standard Mundipagg_Model_Standard
	...
	private function getRequestDataNode() {
		...
		switch ($antifraudProvider) {
			case Mundipagg_Model_Source_Antifraud::ANTIFRAUD_NONE:
			...
			case Mundipagg_Model_Source_Antifraud::ANTIFRAUD_CLEARSALE:
				...
				$sessionId = Mundipagg_Model_Customer_Session::getSessionId();
				break;
	...
	private function clearAntifraudDataFromSession() {
		...
		$customerSession->unsetData(Mundipagg_Model_Customer_Session::SESSION_ID);
	}

	/**
	 * Method to unify the transactions requests and his logs
	 * @todo must be deprecated, use Mundipagg_Model_Api::sendJSON method
	 *
	 * @since 05-24-2016
	...
	private function saveOfflineRetryStatements($orderIncrementId, DateTime $createDate) {
		...
		if (Mundipagg_Model_Offlineretry::offlineRetryIsEnabled()) {
			...
			$model = new Mundipagg_Model_Offlineretry();
			$offlineRetry = $model->loadByIncrementId($orderIncrementId);
	...
	public function getOrderTransactions($orderReference) {
		...
		$util = new Mundipagg_Helper_Util();
