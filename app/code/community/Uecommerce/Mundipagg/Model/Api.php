...
//					$trans = $creditCardTransactionResultCollection['CreditCardTransactionResult'];
        ...
        try {
            ...
            switch ($lowerStatus) {
                ...
                case 'captured':
                    ...
                    return $this->processCapturedStatus(
                            $order,
                            $amountToCapture,
                            $transactionKey,
                            $orderReference,
                ...
                case 'underpaid':
                    ...
                    return $this->processUnderpaidStatus(
                            $order,
                            $helperLog,
                            $returnMessageLabel,
                            $capturedAmountInCents,
                ...
                case 'notauthorized':
                    ...
                    return $this->processNotAuthorizedStatus(
                        $order,
                        $transactionData,
                        $returnMessageLabel,
                        $helperLog
                ...
                case 'authorizedpendingcapture':
                    ...
                    return $this->processAuthorizedPendingCaptureStatus(
                        $order,
                        $helperLog,
                        $status
                    );
                ...
                case $statusWithError:
                    ...
                    return $this->processWithErrorStatus(
                        $order,
                        $helperLog,
                        $returnMessageLabel
                    );
     ...
     */
    private function processCapturedStatus($order, $amountToCapture, $transactionKey, $orderReference, $helperLog)
    {
        try {
     ...
     * from notification post
     */
    private function processPaidStatus($order, $helperLog, $returnMessageLabel, $status, $capturedAmountInCents, $data)
    {
        if ($order->canUnhold()) {
            $order->unhold();
     ...
     * from notification post.
     */
    private function processUnderpaidStatus($order, $helperLog, $returnMessageLabel, $capturedAmountInCents, $status)
    {
        if ($order->canUnhold()) {
            $helperLog->info("{$returnMessageLabel} | unholded.");
            $order->unhold();
     ...
     * from notification post.
     */
    private function processNotAuthorizedStatus($order, $transactionData, $returnMessageLabel, $helperLog)
    {
        $helper = Mage::helper('mundipagg');
        $grandTotal = $order->getGrandTotal();
     ...
     */
    private function processCanceledStatus($order, $returnMessageLabel, $helperLog, $status)
    {
        if ($order->canUnhold()) {
            $helperLog->info("{$returnMessageLabel} unholded.");
            $order->unhold();
     ...
     * from notification post.
     */
    private function processAuthorizedPendingCaptureStatus($order, $helperLog, $status)
    {
     ...
     * from notification post.
     */
    private function processWithErrorStatus($order, $helperLog, $returnMessageLabel)
    {
        try {
     ...
     * from notification post.
     */
    private function processNotRecognizedStatus($exception, $helperLog)
    {
